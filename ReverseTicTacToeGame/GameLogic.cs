...
namespace ReverseTicTacToeGame
{
    internal class GameLogic
    {
        ...
        public GameLogic(int i_BoardSize, bool i_Player1IsComputer, bool i_Player2IsComputer)
        {
            ...
            m_Player1 = new Player(eSignsOfPlayers.Cross, i_Player1IsComputer);
            m_Player2 = new Player(eSignsOfPlayers.Circle, i_Player2IsComputer);
        ...
        private bool thereIsWin((int row, int column) i_Point, eSignsOfPlayers i_PlayerSign)
        {
            ...
            bool isWinInDiagonal = checkWinInMainDiagonal(i_PlayerSign, numberOfSignsToWin);
            bool isWinInAntiDiagonal = checkWinInAntiDiagonal(i_PlayerSign, numberOfSignsToWin);

            return isWinInDiagonal || isWinInAntiDiagonal || isWinInRowAndCol;
        }

        private bool checkWinInAntiDiagonal(eSignsOfPlayers i_PlayerSign, int i_NumberOfSignsToWin)
        {
            int counter = 0;
            ...
            {
                if(m_GameBoard.GameBoard[i, m_GameBoard.Size - i] == i_PlayerSign)
                {
                    counter++;
                }
                else
        ...
        private bool checkWinInMainDiagonal(eSignsOfPlayers i_PlayerSign, int i_NumberOfSignsToWin)
        {
            int counter = 0;

            for(int i = 1; i < this.GameBoard.Size; i++)
            {
                if(this.GameBoard.GameBoard[i, i] == i_PlayerSign)
                {
                    counter++;
                }
                else
            ...
            int i_NumberOfSignsToWin)
        {
            ...
            for(int i = 1; i < this.GameBoard.Size; i++)
            {
                if(m_GameBoard.GameBoard[i_Point.row, i] == i_PlayerSign)
                {
                    counterRow++;
                }

                if(m_GameBoard.GameBoard[i, i_Point.column] == i_PlayerSign)
                {
                    counterColumn++;
                }
            }
        ...
        {
            bool isInRangeOfBoard = i_Number >= 1 && i_Number < this.GameBoard.Size;
